...
export function createKnownKeysightPreset(args: {
  ...
  const triggerSweepCommandId = makeId("cmd");
  const triggerSourceCommandId = makeId("cmd");
  const triggerLevelCommandId = makeId("cmd");
  const triggerSlopeCommandId = makeId("cmd");
  const setTriggerModeCommandId = makeId("cmd");
  const setTriggerSweepCommandId = makeId("cmd");
  const setTriggerSourceCommandId = makeId("cmd");
  const setTriggerLevelCommandId = makeId("cmd");
  ...
  const device: EquipmentDevice = {
    ...
    transport: "tcp-scpi",
    address: targetAddress ?? "192.168.8.193:5025",
    ...
    commands: [
      ...
      { id: triggerSweepCommandId, name: "Read Trigger Sweep", categoryPath: "Trigger / Read", builtIn: true, mode: "scpi", payload: ":TRIGger:SWEep?", parser: "text", timeoutMs: 3000, artifactMode: "text", notes: "Reads whether acquisition is auto or normal when waiting for triggers.", inputDefs: [], outputDefs: [{ id: makeId("output"), name: "triggerSweep", source: "json-path", selector: "text" }] },
      ...
      { id: triggerLevelCommandId, name: "Read Trigger Level", categoryPath: "Trigger / Read", builtIn: true, mode: "scpi", payload: ":TRIGger:LEVel? CHANnel{{channelNumber}}", parser: "text", timeoutMs: 3000, artifactMode: "text", notes: "Reads the trigger level for a specific analog channel.", inputDefs: [{ id: makeId("input"), name: "channelNumber", required: true, defaultValue: "1" }], outputDefs: [{ id: makeId("output"), name: "triggerLevelText", source: "json-path", selector: "text" }, { id: makeId("output"), name: "triggerLevelVolts", source: "regex", selector: scpiValueRegex, captureGroup: 1 }] },
      ...
      { id: setTriggerModeCommandId, name: "Set Trigger Mode", categoryPath: "Trigger / Configure", builtIn: true, mode: "scpi", payload: ":TRIGger:MODE {{mode}}", parser: "none", timeoutMs: 3000, artifactMode: "none", notes: "Sets the Keysight trigger type. Use EDGE for the fire-hazard CH1 edge trigger setup.", inputDefs: [{ id: makeId("input"), name: "mode", required: true, defaultValue: "EDGE", options: ["EDGE", "GLITch", "PATTern", "CAN", "LIN", "I2C", "SPI", "USB", "VIDeo"] }], outputDefs: [] },
      { id: setTriggerSweepCommandId, name: "Set Trigger Sweep", categoryPath: "Trigger / Configure", builtIn: true, mode: "scpi", payload: ":TRIGger:SWEep {{sweep}}", parser: "none", timeoutMs: 3000, artifactMode: "none", notes: "Sets automatic or normal trigger sweep behavior. AUTO replaces Siglent TRMD AUTO for this scope.", inputDefs: [{ id: makeId("input"), name: "sweep", required: true, defaultValue: "AUTO", options: ["AUTO", "NORMal"] }], outputDefs: [] },
      { id: setTriggerSourceCommandId, name: "Set Trigger Source", categoryPath: "Trigger / Configure", builtIn: true, mode: "scpi", payload: ":TRIGger:EDGE:SOURce CHANnel{{channelNumber}}", parser: "none", timeoutMs: 3000, artifactMode: "none", notes: "Sets the analog channel used as the edge trigger source.", inputDefs: [{ id: makeId("input"), name: "channelNumber", required: true, defaultValue: "1", options: ["1", "2", "3", "4"] }], outputDefs: [] },
      { id: setTriggerLevelCommandId, name: "Set Trigger Level", categoryPath: "Trigger / Configure", builtIn: true, mode: "scpi", payload: ":TRIGger:LEVel {{levelVolts}},CHANnel{{channelNumber}}", parser: "none", timeoutMs: 3000, artifactMode: "none", notes: "Sets the trigger level for a specific analog channel. Keysight expects the voltage before the channel selector.", inputDefs: [{ id: makeId("input"), name: "levelVolts", required: true, defaultValue: "1.50" }, { id: makeId("input"), name: "channelNumber", required: true, defaultValue: "1", options: ["1", "2", "3", "4"] }], outputDefs: [] },
      ...
      { id: setTimebaseCommandId, name: "Set Time Range", categoryPath: "Acquire / Timing", builtIn: true, mode: "scpi", payload: ":TIMebase:RANGe {{timeRangeSeconds}}", parser: "none", timeoutMs: 3000, artifactMode: "none", notes: "Sets the total horizontal window in seconds. The fire-hazard setup uses 7 ms.", inputDefs: [{ id: makeId("input"), name: "timeRangeSeconds", required: true, defaultValue: "7.0E-3" }], outputDefs: [] },
  ...
  const scripts: EquipmentScript[] = [{
    ...
    steps: [
      ...
      makeStep("Read trigger sweep", triggerSweepCommandId, "command", "trigger-sweep.txt"),
