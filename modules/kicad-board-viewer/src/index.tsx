...
import { exportBoardFromSource, validateBoardArtifact } from "kicad-board-export";
import type { BoardArtifact, BoardEdge, BoardNet, BoardPad, BoardPoint, BoardTrack, BoardVia } from "kicad-board-export";
...
export default function KiCadBoardViewer({ config }: ModuleProps) {
  ...
  useEffect(() => {
    ...
    getS3Client(uploadTarget.bucket)
      ...
      .then((text) => {
        if (cancelled) return;
        const parsed = validateBoardArtifact(JSON.parse(text));
        setBoard(parsed);
  ...
  const uploadBoard = useCallback(async (file: File) => {
    ...
    try {
      ...
      let parsed: BoardArtifact;
      let artifactText: string;
      let artifactFilename: string;

      if (lowerName.endsWith(".kicad_pcb")) {
        parsed = exportBoardFromSource(text, {
          filename: file.name,
          exporter: "modules/kicad-board-viewer",
        });
        artifactText = `${JSON.stringify(parsed, null, 2)}\n`;
        artifactFilename = file.name.replace(/\.kicad_pcb$/i, ".board-viewer.json");
      } else if (lowerName.endsWith(".json")) {
        parsed = validateBoardArtifact(JSON.parse(text));
        artifactText = `${JSON.stringify(parsed, null, 2)}\n`;
        artifactFilename = file.name;
      } else {
        throw new Error("Upload either a .kicad_pcb file or an exported board-viewer .json artifact.");
      }

      const key = `${uploadTarget.prefix}/${Date.now().toString(36)}-${artifactFilename}`;
      ...
      const nextMeta = { key, filename: parsed.source?.file ?? file.name };
      ...
      <div style={dropShellStyle}>
        ...
        >
          ...
          <p style={{ margin: 0, color: C.text, fontSize: "0.95rem", fontWeight: 700 }}>
            Drop a .kicad_pcb file or board-viewer JSON artifact, or click to browse
          </p>
          <p style={{ margin: 0, color: C.muted, fontSize: "0.78rem", maxWidth: 520, lineHeight: 1.6, textAlign: "center" }}>
            Raw KiCad boards are converted in-browser during upload. You can still pre-export with: npm run kicad:export-board -- --pcb path/to/board.kicad_pcb --out board-viewer.json
          </p>
        </div>
        <input ref={fileInputRef} type="file" accept=".kicad_pcb,.json,application/json" onChange={handleFileChange} style={{ display: "none" }} />
      </div>
    );
  }
